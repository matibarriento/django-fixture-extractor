...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-fixtures-extractor runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-fixtures-extractor.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ fixtures_extractor")
...
def lint(c):
    ...
    """
    c.run("flake8 django-fixtures-extractor tests")
...
def release(c, bumpsize=''):
    ...
    import django_fixtures_extractor
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_fixtures_extractor.__version__))
    c.run("git push --tags")
